...
def save_loss_curves(loss_dict, output_dir):
    """Saves loss curves and writes losses to a text file."""
    ...
    for loss_type, values in loss_dict.items():
        ...
        plt.legend()
        plt.grid()
        plt.savefig(os.path.join(output_dir, f"{loss_type}_curve.png"))
        plt.close()
    
    # Save all losses to a text file
    with open(os.path.join(output_dir, "losses.txt"), "w") as f:
        for loss_type, values in loss_dict.items():
            f.write(f"{loss_type}: {values}\n")
...
def train(args):
    ...
    # Initialize dictionaries for storing losses
    loss_dict = {
    ...
    for i in range(num_epochs):
        ...
        # Store epoch losses
        loss_dict["RPN_Classification_Loss"].append(np.mean(rpn_classification_losses))
        loss_dict["RPN_Localization_Loss"].append(np.mean(rpn_localization_losses))
        loss_dict["FRCNN_Classification_Loss"].append(np.mean(frcnn_classification_losses))
        loss_dict["FRCNN_Localization_Loss"].append(np.mean(frcnn_localization_losses))
    
    # Save loss curves and losses to files
    save_loss_curves(loss_dict, "loss_curves")
    print('Done Training...')
