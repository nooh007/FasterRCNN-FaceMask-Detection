...
class FasterRCNN(nn.Module):
    def __init__(self, model_config, num_classes):
        ...
        self.backbone = nn.Sequential(
            *(list(resnet.children())[:-2])  # Use up to the last convolutional block
        )
        self.backbone_out_channels = 2048  # ResNet50's final feature map depth
        ...
        for layer in list(self.backbone.children())[:6]:  # Freeze up to Conv3_x
    ...
    def forward(self, image, target=None):
        ...
        if not self.training:
            # Transform boxes to original size during inference
